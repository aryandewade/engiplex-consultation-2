...
export const ConsultantDashboard: React.FC<ConsultantDashboardProps> = ({ onNavigate }) => {
  ...
  const getDisplayedBookings = () => {
    ...
    if (activeTab === 'today') list = bookings.today;
    else if (activeTab === 'upcoming') list = bookings.upcoming;
    else if (activeTab === 'completed') list = bookings.completed;
    else list = bookings.all;
                      ...
                      <div className="space-y-1">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          {booking.isStudentOrFresher ? (
                            <span className="px-2 py-0.5 rounded-full text-[10px] font-semibold bg-blue-950 text-blue-300 border border-blue-800/50 flex items-center gap-1">
